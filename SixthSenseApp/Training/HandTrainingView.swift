...
struct HandTrainingView: View {
    ...
    private var footer: some View {
        ...
        if leftGesture == .fist {
            ...
            leftIcon = "arrow.triangle.2.circlepath"
            leftDescription = "Rolando..."
            leftActive = true
        } else {
            leftIcon = "hand.pinch"
            leftDescription = "Pinça clica   •   Punho arrasta   •   Círculo rola"
            leftActive = false
        }
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(alignment: .top, spacing: 14) {
                    legendItem(icon: "rectangle.on.rectangle", color: .mint,
                               label: "Mission Control", description: "Swipe rápido da mão direita para cima")
